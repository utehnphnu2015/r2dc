...
class ProvinceController extends \yii\web\Controller {
    ...
    public function actionIndex($rep_year = 2015) {// แสดงทุกรายการ kpi เขต
        $sql = "
 SELECT  t.id,t.topic
,( SELECT ROUND(SUM(k.total)*100/SUM(k.target),2)  from kpi_province k WHERE k.kpi_id=t.id AND k.provcode = 53 AND k.rep_year=$rep_year) as  'p53'
,( SELECT ROUND(SUM(k.total)*100/SUM(k.target),2)  from kpi_province k WHERE k.kpi_id=t.id AND k.provcode = 63 AND k.rep_year=$rep_year) as  'p63'
,( SELECT ROUND(SUM(k.total)*100/SUM(k.target),2) from kpi_province k WHERE k.kpi_id=t.id AND k.provcode = 64 AND k.rep_year=$rep_year) as  'p64'
,( SELECT ROUND(SUM(k.total)*100/SUM(k.target),2)  from kpi_province k WHERE k.kpi_id=t.id AND k.provcode = 65 AND k.rep_year=$rep_year) as  'p65'
,( SELECT ROUND(SUM(k.total)*100/SUM(k.target),2)  from kpi_province k WHERE k.kpi_id=t.id AND k.provcode = 63 AND k.rep_year=$rep_year) as  'p67'

FROM topic_province t ";
